...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    var redBuscada = red.toLowerCase().replace(/\s+/g, ' ').trim();
    ...
    var filtrados = data.body.results.filter(function(page) {
      ...
      var statusS = statusNombre.toLowerCase();
      ...
      if (redProp.multi_select && redProp.multi_select.length > 0) {
        redes = redProp.multi_select.map(function(r){ return r.name.toLowerCase(); });
      } else if (redProp.select && redProp.select.name) {
        redes = [redProp.select.name.toLowerCase()];
      }
      var tieneRed = redes.some(function(r){
      ...
      if (formatoProp.multi_select && formatoProp.multi_select.length > 0) {
        formatos = formatoProp.multi_select.map(function(f){ return f.name.toLowerCase(); });
      } else if (formatoProp.select && formatoProp.select.name) {
        formatos = [formatoProp.select.name.toLowerCase()];
      }
      var tieneFormato = formatos.some(function(f){
        return f.indexOf('reel') !== -1 || f.indexOf('post') !== -1 || f.indexOf('carrusel') !== -1;
      });
    ...
    var pages = filtrados.slice(0, 9).map(function(page) {
      ...
      page._formato = formatoNombre.toLowerCase();
